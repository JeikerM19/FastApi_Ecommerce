...
def agregar_producto(producto_id: int, db:Session = Depends(get_db), cantidad: int = 1, user=Depends(get_current_user)):
    carrito = crud_carrito.obtener_carrito(db, user.id)
...
def eliminar_item(item_id: int, db: Session = Depends(get_current_user), user = Depends(get_current_user)):
